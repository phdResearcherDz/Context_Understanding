...
def main_cl_enhanced():
    result_folder = f"results/results_cl_enhanced"
    ...
    for dataset in datasets:
        path_data_enhanced_train = f"5_formated_metadata/train.json"
        path_data_enhanced_test = f"5_formated_metadata/test.json"
        first_half_train_data,second_half_train_data  = split_dataset_file(dataset,file_name=path_data_enhanced_train,output_file_type="train")

        val_data = load_dataset_yes_no(dataPath + f"{dataset}/"+path_data_enhanced_test)
        test_data = load_dataset_yes_no(dataPath + f"{dataset}/"+path_data_enhanced_test)
        for model_name in models:
            ...
            with open(result_continual_file_path, "w") as result_continual_file:
                ...
                result_continual_file.write(
                    f"Overall mean test accuracy after {num_iterations} iterations: {overall_mean_acc}")
