...
def get_item_metadata_with_deffinition_relation(context_entities, question_entities, primekg, hetionet):
    ...
    relations_dict = {}
    for relation in (primekg + hetionet):
...
def get_item_metadata_with_relation(context_entities, question_entities, primekg,
                                                hetionet):
    # Get all concepts with definitions
    concepts_def = get_all_concepts_with_definition(context_entities, question_entities)
    definitions_text = ""
...
def get_item_metadata_with_relation_summurize(context_entities, question_entities, primekg,
                                                hetionet,context):
    # Get all concepts with definitions
    concepts_def = get_all_concepts_with_definition(context_entities, question_entities)
    definitions_text = ""
...
def get_item_metadata_with_deffinition(context_entities, question_entities, primekg,
                                       hetionet):
    # Get all concepts with definitions
    concepts_def = get_all_concepts_with_definition(context_entities, question_entities)
    definitions_text = ""
...
def get_item_metadata_with_deffinition_summarize(context_entities, question_entities, primekg,
                                       hetionet,context):
    # Get all concepts with definitions
    concepts_def = get_all_concepts_with_definition(context_entities, question_entities)
    definitions_text = ""
...
def get_item_metadata_with_deffinition_relation_summarize(context_entities, question_entities, primekg,
                                                hetionet,context):
    # Get all concepts with definitions
    concepts_def = get_all_concepts_with_definition(context_entities, question_entities)
    definitions_text = ""
...
def process_json(json_file_path,method,version_filter,kg):
    ...
    for item in tqdm(data):
        ...
        match kg:
            case "both":
                primekg_relations = item[f"primekg_{version_filter}_relations"]
                hetionet_relations = item[f"hetionet_{version_filter}_relations"]
            case "primekg":
                primekg_relations = item[f"primekg_{version_filter}_relations"]
                hetionet_relations = []
            case "hetionet":
...
def process_json_medqa(json_file_path, method, version_filter, kg):
    ...
    for item in tqdm(data):
        ...
        context_entities = item["context_entities"]
        question_entities = item["question_entities"]
        if version_filter == "tfidf":
            version_filter = "tfidf_weighted"
        ...
        keys_to_remove = [
            f"primekg_{version_filter}_relations",
            f"hetionet_{version_filter}_relations",
...
def process_dataset(dataset,type_formating,version_filter,kg="both"):
    directory_path = f'{root_folder}/Pre_Processed_Datasets/{dataset}/4_extracted_concepts_relations_{version_filter}/'
