...
def batch_similarity_check(vectorizer, df_nodes, words):
    node_vectors = vectorizer.transform(df_nodes['name'])
...
def get_node_details(driver, node_name):
    with driver.session() as session:
        ...
        result = session.run("MATCH (n {name: $node_name})-[r]->(m) RETURN type(r) as relation, collect(m.name) as nodes", node_name=node_name)
...
def process_json(driver, file_path, df_nodes):
    ...
    vectorizer.fit(df_nodes['name'])  # Fit only on node names
    node_vectors = vectorizer.transform(df_nodes['name'])
    ...
    for item in tqdm(data):
        ...
        if context == None or context == '':
            continue
        context_entities = set(item.get('context_entities', []))
        if (len(context_entities) == 0):
            item["medical_concepts_hetionet"] = []
        ...
        most_similar_nodes = df_nodes['name'][similarities.argmax(axis=1)]
        ...
        item["medical_concepts_hetionet"] = list(medical_concepts)
        new_data.append(item)
...
def process_dataset(driver, dataset, df_nodes):
    ...
    for json_file_path in glob.glob(directory_path + '*.json'):
        ...
        new_directory = f"../Pre_Processed_Datasets/{dataset}/2_extracted_concepts/data"
        os.makedirs(new_directory,exist_ok=True)

        new_filename = os.path.splitext(filename)[0] + "_with_concept.json"
